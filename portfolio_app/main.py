...
def main(page: ft.Page):
    ...
    # --------------------------------------------------
    # DOCUMENT VIEWER STATE (shared across tabs)
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    def make_drive_embed_url(url: str) -> str:
        try:
            if "/d/" in url:
    ...
    def display_doc(name: str, doc_type: str, url: str):
        if url.startswith("YOUR_GOOGLE_DRIVE"):
            doc_viewer_title.value = "No link set"
            doc_viewer_desc.value = f"Please replace the placeholder URL for {name}."
            doc_viewer_frame.content = ft.Text(
                "⚠️ Placeholder URL — add a real Google Drive link.",
                color="red400",
            )
            page.update()
            return
        if url.startswith("http"):
            embed_url = make_drive_embed_url(url)
        ...
        doc_viewer_desc.value = "Displaying document below:"
        page.update()
    ...
    # --------------------------------------------------
    # LOG CARD (for GitHub Evidence section)
    # --------------------------------------------------
    def log_card(title: str, url: str) -> ft.Container:
        def on_view(e, u=url):
            display_doc(title, "Log", u)
    ...
    def build_timeline_tab():
        def week_entry(label: str, desc: str):
            return ft.Column([
                    ...
                    ft.Container(
                        content=ft.Column(
                            [
                                ...
                                ft.Text(
                                    "Replace this container with:\nft.WebView(url='https://www.youtube.com/embed/YOUR_VIDEO_ID')",
                                    size=12,
                                    color="grey600",
                                    text_align=ft.TextAlign.CENTER,
                                ),
